...
class Marca extends CI_Controller {
	...
	public function add_db_(){
	    /*if (move_uploaded_file($_FILES["file"]["tmp_name"], $_SERVER['DOCUMENT_ROOT']."/menudigital/Marcas/".$_FILES['file']['name']))
	    echo 'correcto';
	    else 'error';
	    print_r ($_SERVER['DOCUMENT_ROOT']."/menudigital/Marcas/");*/
	    if($_FILES['file']['name'])
	     echo '1';
	    else echo '2';
    ...
    public function add_db(){
		...
		$response = array();    
		if (move_uploaded_file($_FILES["file"]["tmp_name"], $_SERVER['DOCUMENT_ROOT']."/menudigital/Marcas/".$nuevo)) {
...
public function update_db(){
		...
		if(count($res) == 0){
			...
			{	
				...
				$data['imagen'] = $nuevo;
				if (move_uploaded_file($temporal, $_SERVER['DOCUMENT_ROOT']."/menudigital/Marcas/".$nuevo)) {
					$envio_imagen = true;
				}else
				{
						$response["status"] = "ERROR";
