...
class Categoria extends CI_Controller
{
	...
	public function add_db()
	{
		...
		if (move_uploaded_file($_FILES["file"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/menudigital/categorias/" . $nuevo)) {

			if (move_uploaded_file($_FILES["imagen_titulo_categoria"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/menudigital/categorias/" . $nuevo_titulo)) {

				if ($envio_imagen_fondo) {
					if (move_uploaded_file($temporal_fondo, $_SERVER['DOCUMENT_ROOT'] . "/menudigital/categorias/" . $nuevo_fondo)) {
	...
	public function update_db()
	{
		...
		if (count($res) == 0) {
			...
			if ($nombre != "") {
				...
				$data['imagen'] = $nuevo;
				if (move_uploaded_file($temporal, $_SERVER['DOCUMENT_ROOT'] . "/menudigital/categorias/" . $nuevo)) {
					$envio_imagen = true;
				} else {
					$response["status"] = "ERROR";
					$response["respuesta"] = "Error al cargar archivo";
			...
			if ($nombre_titulo != "") {
				...
				if (move_uploaded_file($temporal_titulo, $_SERVER['DOCUMENT_ROOT'] . "/menudigital/categorias/" . $nuevo_titulo)) {
					$envio_imagen_titulo = true;
				} else {
					$response["status"] = "ERROR";
					$response["respuesta"] = "Error al cargar archivo";
