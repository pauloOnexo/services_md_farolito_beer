...
class Experiencia extends CI_Controller {
    ...
    public function add_db(){
		...
		$response = array();    
		if (move_uploaded_file($_FILES["file"]["tmp_name"], getenv('DIR_FILES')."/menudigital/Experiencias/".$nuevo)) {
			if (move_uploaded_file($_FILES["imagen_popup"]["tmp_name"], getenv('DIR_FILES')."/menudigital/Experiencias/".$nuevo_popup)) {
...
public function update_db(){
		...
		if(count($res) == 0){
			...
			{	
				...
				$data['imagen'] = $nuevo;
				if (move_uploaded_file($temporal, getenv('DIR_FILES')."/menudigital/Experiencias/".$nuevo)) {
					$envio_imagen = true;
				}else
				{
						$response["status"] = "ERROR";
			...
			{	
				...
				if (move_uploaded_file($temporal_popup, getenv('DIR_FILES')."/menudigital/Experiencias/".$nuevo_popup)) {
					$envio_imagen = true;
				}else
				{
						$response["status"] = "ERROR";
	...
	function experiencia_marca() {  
		...
		if(isset( $_GET['id_idioma']) && $id_idioma > 0){/*Usado para traducción en beerfactory*/
			$response["registros"] = $this->Experiencia_model->getAllByLanguage($id_marca, $id_idioma);
			echo json_encode($response);
			return;
		}
