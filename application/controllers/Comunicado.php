...
class Comunicado extends CI_Controller {
	...
	public function QR(){
		...
		if(count($res) == 0){
			...
			$PNG_TEMP_DIR = getenv('DIR_FILES').DIRECTORY_SEPARATOR."menudigital".DIRECTORY_SEPARATOR."QR".DIRECTORY_SEPARATOR;
			$response["status"] = "OK";
			$response["respuesta"] = "Ingreso correcto";
	...
	public function QR_Sucursales(){
		...
		$filename = $PNG_TEMP_DIR."unidad"."_".$unidad.".jpg";

		$contenido = "https://www.menugrg.com.mx:7443/elfarolito/?unidad=".$unidad;
		$archivo='menudigital/QR/'."unidad".'_'.$unidad.".jpg";


		QRcode::png($contenido,$filename,QR_ECLEVEL_L,10,2);
	...
	public function QR_Combinaciones(){
		...
		if(count($res) == 0){
			...
			foreach($unidades as $unidad){
				...
				if(file_exists(getenv('DIR_FILES').'/menudigital/QR/'.$nombre_region.($alcohol ==1 ? $nombre_alcohol:'').($individual =='1' ?'_individual':'').($kilo ==1 ?'_kilo':'').'_'.$numero.".jpg"))
					unlink(getenv('DIR_FILES').'/menudigital/QR/'.$nombre_region.($alcohol ==1 ? $nombre_alcohol:'').($individual =='1' ?'_individual':'').($kilo ==1 ?'_kilo':'').'_'.$numero .".jpg");
				$response["status"] = "OK";
				$response["respuesta"] = "Ingreso correcto";
