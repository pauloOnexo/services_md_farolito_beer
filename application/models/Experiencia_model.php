...
class Experiencia_model extends CI_Model {
	...
	public function getAllByLanguage($id_marca, $id_idioma)
	{
		$query = $this->db->query("SELECT * FROM v_experiencias_por_idioma where id_marca = ".$id_marca." AND id_idioma = ".$id_idioma);
		return $query->result();
	}
    ...
    public function eliminar($id){
        ...
        $ruta ="";
        foreach($result->result() as $column){
			$ruta = getenv('DIR_FILES')."/menudigital/Experiencias/". $column->imagen;
		}
