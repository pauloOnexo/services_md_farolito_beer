...
class Categoria_model extends CI_Model {
	...
	public function get_activas_sucursal($id_sucursal,$id_marca,$kilos)
	{
		...
		// Obteniendo las restricciones por categoria
		// foreach($sucursal->result() as $column){
        //     $restricciones = $column->res_cats;
		// }
		// 
	    $complemento ='';
	    $consulta ='';
		...
		//                             where 1=1 and activo = 1 and a.id not in ($restricciones) and b.id_marca=".$id_marca.$complemento_sinkilos." order by orden";
		...
		elseif ($id_sucursal != 231) {
			$consulta = "select distinct (case when a.id=47 then 0 else a.id end) orden, a.id id_categoria,descripcion descripcion_categoria
			,nombre nombre_categoria,a.id_marca,a.ubicacion ubicacion_categoria ,imagen_titulo_categoria,a.imagen_fondo
		                            from Categoria a join Categoria_Marca b on b.id_categoria = a.id 
		                            where 1=1 and activo = 1 and a.id not in (71) and b.id_marca=".$id_marca.$complemento_sinkilos." order by orden";
		}
		else{
			$consulta = "select distinct (case when a.id=47 then 0 else a.id end) orden, a.id id_categoria,descripcion descripcion_categoria
			,nombre nombre_categoria,a.id_marca,a.ubicacion ubicacion_categoria ,imagen_titulo_categoria,a.imagen_fondo
    ...
    public function eliminar($id){
        ...
        $ruta ="";
        foreach($result->result() as $column){
			$ruta = getenv('DIR_FILES')."/menudigital/categorias/". $column->imagen;
		}
        $this->db->where('id', $id);
