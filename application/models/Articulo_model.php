...
class Articulo_model extends CI_Model {
	...
	public function getById($id){
        ...
        foreach($result->result() as $column){
			...
			$response["precio_colegio_porcentaje"] =round( $column->precio_colegio_porcentaje,0);
			$response["precio_colegio_cuota"] =round( $column->precio_colegio_cuota,0);
			$response["precio_aeropuerto_aifa"] =round( $column->precio_aeropuerto_aifa,0);
			$response["precio_tezontle"] =round( $column->precio_tezontle,0);
			...
			$response["vigencia"] = $column->vigencia;									   
			$response["fechainicio"] = $column->fechainicio;									   
			$response["fechafin"] = $column->fechafin;									   
		}
        return $response;
    }

    // Actualizacion farolito 30/09
    public function getSucursalesRestringidas($id_articulos){

        $result = $this->db->query("select id from Sucursal where FIND_IN_SET($id_articulos,res_arts)");
    ...
    public function get_medidas($id){
        $result = $this->db->query("SELECT  a.selected,a.id,a.numero,a.id_articulo,a.nombre nombre_medida,a.precio_nacional,a.precio_acapulco,a.precio_cc,a.precio_tijuana,a.precio_pl,a.cantidad_x_porcion_medida,a.precio_aeropuerto,a.precio_colegio_porcentaje,a.precio_colegio_cuota,a.precio_aeropuerto_aifa,a.precio_tezontle,a.activo 
        FROM `Articulo_Medida` a where a.id_articulo = ".$id);
        return $result->result();
    }
	public function get_medidas_idioma($id,$id_idioma){
		$registrosI = $this->db->query("SELECT  a.selected,a.id,a.numero,a.id_articulo,a.nombre nombre_medida,a.precio_nacional,a.precio_acapulco,a.precio_cc,a.precio_tijuana,a.precio_pl,a.cantidad_x_porcion_medida,a.precio_aeropuerto,a.precio_colegio_porcentaje,a.precio_colegio_cuota,a.precio_tezontle,a.activo 
		...
		if ($existe == 0)
		{
			$result = $this->db->query("SELECT  a.selected,a.id,a.numero,a.id_articulo,a.nombre nombre_medida,a.precio_nacional,a.precio_acapulco,a.precio_cc,a.precio_tijuana,a.precio_pl,a.cantidad_x_porcion_medida,a.precio_aeropuerto,a.precio_colegio_porcentaje,a.precio_colegio_cuota,a.precio_tezontle,a.activo, a.id id_articulo_medida 
			FROM `Articulo_Medida` a where a.id_articulo = ".$id);
			return $result->result();
		}
		else{
			...
			,a.cantidad_x_porcion_medida,a.precio_aeropuerto,a.precio_colegio_porcentaje,a.precio_colegio_cuota,a.precio_tezontle,a.activo, a.id_articulo_medida 
			FROM `Articulo_Medida_Idioma` a where a.id_articulo = ".$id. ' and a.id_idioma='.$id_idioma." 
			union all 
			SELECT  a.selected,a.id,a.numero,a.id_articulo,a.nombre nombre_medida
			,a.precio_nacional,a.precio_acapulco,a.precio_cc,a.precio_tijuana,a.precio_pl
			,a.cantidad_x_porcion_medida,a.precio_aeropuerto,a.precio_colegio_porcentaje,a.precio_colegio_cuota,a.precio_tezontle,a.activo, a.id
	...
	public function get_medidas_precio($id,$precio,$id_sucursal,$id_subcategoria,$nombre){
        ...
        $result = $this->db->query("select selected,id,numero,id_articulo,nombre_medida
            ...
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 9 then ROUND(a.precio_colegio_porcentaje)
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 10 then ROUND(a.precio_colegio_cuota)
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 7 then ROUND(a.precio_tezontle)
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 80 then ROUND(a.precio_aeropuerto_aifa)
             end ) precio
    ...
    public function get_medidas_precio_idioma($id,$precio,$id_sucursal,$id_subcategoria,$nombre,$id_idioma){
		...
		if($alcohol == 4 && $id_subcategoria== 27  )
            ...
            //when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 2 then (select round(aa.precio_acapulco) from Articulo_Medida aa where a.id_articulo = aa.id_articulo OR A.id_articulo_medida = aa.id)
        $result = $this->db->query("
    ...
    public function categoria_subcategorias($id_categoria,$id_sucursal){
		...
		if($region > 0){
			...
			when nombre like '%sabores%' then -1 
			  when a.id = 4907 then 0
			  when a.id = 489 then 1 
			  when a.id = 2 then 2
			  when a.id = 4918 then 3
              when a.id = 488 then 3 
			  when a.id = 4916 then 4
			  when a.id = 4905 then 5
			  when a.id = 4917 and 6=".$region." then 97 
			  when a.id = 4917 and 8=".$region." then 1 
			  when a.id = 15 then 6 
        ...
        return $result->result();
         ...
         if($id_categoria == 2  ){
            $result2 = $this->db->query('select * from ('.$complemento."select * from (select a.id id_subcategoria,a.nombre nombre_subcategoria, a.descripcion descripcion_subcategoria
            ,(
			case when nombre like '%sabores%' then -1 
			when nombre like '%especialidad%' then -1 
			when nombre like '%TOKS a%' then 100 
			when nombre like '%bebidas calientes%' then (100-5)  
			when nombre like '%vinos%' then (100-2)  
			when nombre like '%cervezas%' then (100-3) 
			when nombre like '%refrescantes%' then (100-4) 
			when nombre like '%postres%' then (100-6) else a.id end)orden
            ,a.destacado,a.color_pleca
			from Subcategoria a 
            where FIND_IN_SET('".$id_categoria."',a.categorias) and FIND_IN_SET('0',a.regiones) and a.activo = 1
            group by orden )t0  ) t2 order by orden");
		...
		if($id_categoria == 32){ //comidas aeropuerto
            ...
            ,(case  when nombre like '%cervezas%' then (100)  when nombre like '%refrescantes%' then (99) 
			 when nombre like '%especialidad%' then -1 when nombre like '%postres%' then (97) when nombre like '%bebidas calientes%' then 98
			 when nombre like '%enchiladas%' then 96 
			 when nombre like '%vinos%' then 102 else a.id end)orden
            ,a.destacado,a.color_pleca
			from Subcategoria a 
            where FIND_IN_SET('".$id_categoria."',a.categorias) and FIND_IN_SET('0',a.regiones) and a.activo = 1
            group by orden )t0 ");
		...
		if( $id_categoria == 3 ){
            $result2 = $this->db->query('select * from ('.$complemento."select * from (select a.id id_subcategoria,a.nombre nombre_subcategoria, a.descripcion descripcion_subcategoria
            ,(case when nombre like '%especialidad%' then -1  
			when nombre like '%sabores%' then -1 
			when nombre like '%TOKS a%' then 100 
			when nombre like '%bebidas calientes%' then (100-5)  
			when nombre like '%vinos%' then (100-2)  
		...
		if( $id_categoria == 33){ //cenas aeropuerto
            $result2 = $this->db->query("select * from (select a.id id_subcategoria,a.nombre nombre_subcategoria, a.descripcion descripcion_subcategoria
            ,(case  when nombre like '%cervezas%' then (100)  when nombre like '%refrescantes%' then (99) 
			when nombre like '%postres%' then (97) when nombre like '%bebidas calientes%' then 98 
			when nombre like '%enchiladas%' then 96 when nombre like '%vinos%' then 101 else a.id end)orden
            ,a.destacado,a.color_pleca
			from Subcategoria a 
            where FIND_IN_SET('".$id_categoria."',a.categorias) and FIND_IN_SET('0',a.regiones) and a.activo = 1
            group by orden )t0 ");
        ...
        if($id_categoria == 7){ //bar
            $result2 = $this->db->query('select * from ('.$complemento."select * from (select a.id id_subcategoria,a.nombre nombre_subcategoria, a.descripcion descripcion_subcategoria
            ,(case when nombre like '%vino%' then -1 when a.id = 4907 then 100
			  when a.id = 27 then 0 when a.id = 2 then 2
              when a.id = 38 then 3 
			  when a.id = 39 then 4
		...
		if( $id_categoria == 47){ ///platillos del mes
            ...
            ,(case when a.id = 489 then 1 
              when a.id = 4920 then 2 
              when a.id = 4918 then 2 
              when a.id = 488 then 3 
    ...
    public function categoria_subcategorias_idioma($id_categoria,$id_sucursal,$id_idioma){
        ...
        return $result->result();
         ...
         if($id_categoria == 2  ){
            ...
            ,(case when aa.nombre like '%especialidad%' then -1 
			when aa.nombre like '%TOKS a%' then 100 
			when aa.nombre like '%bebidas calientes%' then (100-5)  
			when aa.nombre like '%vinos%' then (100-2)  
			when aa.nombre like '%cervezas%' then (100-3) 
			when aa.nombre like '%refrescantes%' then (100-4) 
			when aa.nombre like '%postres%' then (100-6) else aa.id end)orden
            ,a.destacado,aa.color_pleca
            from Subcategoria_Idioma a 
			join Subcategoria aa on a.id_subcategoria = aa.id
            where FIND_IN_SET('".$id_categoria."',a.categorias) and a.id_idioma = ".$id_idioma." and FIND_IN_SET('0',a.regiones) and a.activo = 1
		...
		if($id_categoria == 32){
            ...
            ,(case when aa.nombre like '%cervezas%' then (100)  when aa.nombre like '%refrescantes%' then (99) 
			 when aa.nombre like '%especialidad%' then -1 when aa.nombre like '%postres%' then (97) when aa.nombre like '%bebidas calientes%' then 98  
			 when aa.nombre like '%enchiladas%' then 96 when aa.nombre like '%vino%' then 101 else aa.id end)orden
            ,a.destacado,aa.color_pleca
            from Subcategoria_Idioma a 
			join Subcategoria aa on a.id_subcategoria = aa.id
		...
		if( $id_categoria == 33){
            $result2 = $this->db->query("select * from (select aa.id id_subcategoria,a.nombre nombre_subcategoria, a.descripcion descripcion_subcategoria
            ,(case when aa.nombre like '%cervezas%' then (100)  when aa.nombre like '%refrescantes%' then (99) 
			when aa.nombre like '%postres%' then (97) when aa.nombre like '%bebidas calientes%' then 98  
			when aa.nombre like '%enchiladas%' then 96 when aa.nombre like '%vino%' then 101 else aa.id end)orden
            ,a.destacado,aa.color_pleca
            from Subcategoria_Idioma a 
			join Subcategoria aa on a.id_subcategoria = aa.id
        ...
        if($id_categoria == 11){
            ...
            from Subcategoria_Idioma a 
			join Subcategoria aa on a.id_subcategoria = aa.id
            where FIND_IN_SET('".$id_categoria."',a.categorias) and a.id_idioma = ".$id_idioma." and a.activo = 1
            group by orden) t0 ");
        ...
        if($id_categoria == 30 ){
            ...
            from Subcategoria_Idioma a 
			join Subcategoria aa on a.id_subcategoria = aa.id
            where FIND_IN_SET('".$id_categoria."',a.categorias) and a.id_idioma = ".$id_idioma." and FIND_IN_SET('0',a.regiones) and a.activo = 1 and NOT aa.id = 92
            group by orden) t0 ");
		...
		if( $id_categoria == 47){ ///platillos del mes
            ...
            ,(case when aa.id = 489 then 1 
              when aa.id = 4920 then 2
              when aa.id = 2 then 2
              when aa.id = 4918 then 3 
              when aa.id = 488 then 3 
 ...
 public function articulo_subcategorias2($id_subcategoria,$id_sucursal){
		...
		$complemento ='';
		$complemento2 ='';
            // Hot fix 29/08 mostrar producto solo en la región de Altata/Polanco
        ...
        foreach($sucursal->result() as $column){
            ...
            // Hot fix 29/08 mostrar producto solo en la región de Altata/Polanco
            $region = $column->id_region;
            $restricciones = $column->res_arts;
            $sucursal_marca = $column->id_marca;

		}
		if($alcohol == 1)//vc botella
		    $complemento = ' and a.id_subcategoria not in (4917,4922) and a.id not in (1984,1985,1989,1990)';
		if($alcohol == 2) //sin alcohol
		    $complemento = ' and a.id_subcategoria not in (27,28,4917) and a.id not in (1984,1985,1989,1990)';
		if($id_sucursal == 225)   //interlomas
			$complemento = ' and a.id_subcategoria not in (110,111) and a.id not in (1984,1985,1989,1990)';
		if($alcohol == 4) //cerveza
		    $complemento = ' and a.id_subcategoria not in (28,4917) and a.id not in (1984,1985,1989,1990)';
		if($alcohol == 3 && $id_subcategoria == 110)
		    $complemento = ' and a.id in (1984,1985,1989,1990) and a.id_subcategoria not in (111)';
		/*if($alcohol == 3 && $id_subcategoria == 111)
		    $complemento = ' and a.id_subcategoria not in (111)';*/
		/*if($id_estado != 9 && $id_estado != 15)
		    $complemento2 = ' and a.id not in (308)'; //pan frances*/
            // Hot fix 29/08 mostrar producto solo en la región de Altata/Polanco
        // if($region != 34){
        //     $complemento3 = ' and a.id not in(2166,2167,2168)';
        // }

        // Actualización el farolito
        //echo($restricciones);
        if ($restricciones != null) {
            // Caso cuando existen restricciones en una sucursal
            ...
            a.descripcion_imagen,a.detalle_imagen,vigencia,fechainicio,fechafin
            from Articulo a
            left join Experiencia e on a.id_experiencia = e.id
            where (a.activo = 1 and a.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2." and vigencia <> 1 and a.id not in ($restricciones)) or (a.vigencia = 1 and a.activo = 1 and CURDATE() between a.fechainicio and fechafin and  a.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2.") )t0 where precio > 0 order by orden");
        ...
        if ($id_sucursal != 220 && $id_sucursal != 221) {
            $result = $this->db->query("select * from (select a.cantidad_x_porcion,a.orden,".$id_sucursal. "
			,a.extra,a.etiqueta_extra,a.numero_opciones
            ,a.id id_articulo,a.platillo,a.nombre nombre_articulo,a.descripcion descripcion_articulo,a.simbologia,a.logo,a.id_subcategoria,a.id_experiencia,
             a.ubicacion ubicacion_articulo,IF(a.imagen_min IS NULL OR a.imagen_min = '' or a.imagen_min = '/menudigital/Articulos/','',CONCAT('/menudigital/Articulos/'".",a.imagen_min))  ubicacion_articulo_min,(case when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.") in (1)then ROUND(a.precio_nacional )
            ...
            from Articulo a
            left join Experiencia e on a.id_experiencia = e.id
            where (a.activo = 1 and a.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2." and vigencia <> 1 and a.id not in (2166,2167,2168)) or (a.vigencia = 1 and a.activo = 1 and CURDATE() between a.fechainicio and fechafin and  a.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2.") )t0 where precio > 0 order by orden");
            return $result->result();
        }
		...
		$result = $this->db->query("select * from (select a.cantidad_x_porcion,a.orden,".$id_sucursal. "
            ...
            from Articulo a
            left join Experiencia e on a.id_experiencia = e.id
            where (a.activo = 1 and a.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2." and vigencia <> 1) or (a.vigencia = 1 and a.activo = 1 and CURDATE() between a.fechainicio and fechafin and  a.id_subcategoria = ".$id_subcategoria. " ". $complemento." ".$complemento2.") )t0 where precio > 0 order by orden");
            // Hot fix 29/08 mostrar producto solo en la región de Altata/Polanco
	...
	public function articulo_subcategorias2_idioma($id_subcategoria,$id_sucursal,$id_idioma){
		...
		if($alcohol == 2)
		    $complemento = ' and aa.id_subcategoria not in (27,28,4917)';
		    
		if($alcohol == 4)
		    $complemento = ' and aa.id_subcategoria not in (28,4917)';
		...
		$result = $this->db->query("select * from (select a.cantidad_x_porcion,aa.orden
			...
			,IF( a.imagen_min<>'',  CONCAT('/menudigital/Articulos/',a.imagen_min),  IF(aa.imagen_min<>'', CONCAT('/menudigital/Articulos/',aa.imagen_min), aa.ubicacion))  ubicacion_articulo_min
            ...
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 9 then ROUND(aa.precio_colegio_porcentaje)
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 10 then ROUND(aa.precio_colegio_cuota)
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 7 then ROUND(aa.precio_tezontle)
             end ) precio,e.nombre nombre_experiencia, e.descripcion descripcion_experiencia,e.ubicacion ubicacion_experiencia
            ,a.descripcion_imagen,a.detalle_imagen, aa.vigencia, aa.fechafin,aa.fechainicio
            from Articulo_Idioma a
			join Articulo aa on a.id_articulo = aa.id
            left join Experiencia e on aa.id_experiencia = e.id
            where (aa.activo = 1 and a.id_idioma = ".$id_idioma." and aa.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2." and aa.vigencia <> 1) or (aa.vigencia = 1 and CURDATE() between aa.fechainicio and aa.fechafin and aa.activo = 1 and a.id_idioma = ".$id_idioma." and aa.id_subcategoria = ".$id_subcategoria. " ". $complemento."  ".$complemento2." ) 
    ...
    public function articulo_sin_subcategorias($id_categoria,$id_sucursal){
		$complemento ='';
        if($id_sucursal == 219 || $id_sucursal == 214)
            $complemento =' and a.id not in (704) '; 
        $sucursal = $this->db->query("SELECT * FROM Sucursal a where a.id = ".$id_sucursal);
        $alcohol = 0;   
        $result = $this->db->query("select * from (select a.cantidad_x_porcion,a.orden,
            a.id id_articulo,a.platillo,a.nombre nombre_articulo,a.descripcion descripcion_articulo,a.simbologia,a.logo,a.id_subcategoria,a.id_experiencia,
             a.ubicacion  ubicacion_articulo,IF(a.imagen_min IS NULL OR a.imagen_min = '','',CONCAT('/menudigital/Articulos/'".",a.imagen_min))  ubicacion_articulo_min,(case when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.") in (1)then a.precio_nacional 
            ...
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 9 then a.precio_colegio_porcentaje
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 10 then a.precio_colegio_cuota
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 7 then a.precio_tezontle
             end ) precio,
            a.descripcion_imagen,a.detalle_imagen
            from Articulo a
            left join Experiencia e on a.id_experiencia = e.id
            where a.activo = 1 and a.id_categoria = ".$id_categoria. $complemento." )t0 where precio > 0 order by orden");
        
            //where a.activo = 1 and a.id_subcategoria = ".$id_subcategoria." and c.id = ".$id_sucursal);
        return $result->result();
    }

    public function articulo_sin_subcategorias_traduccion($id_categoria,$id_sucursal){
		$complemento ='';
        if($id_sucursal == 219 || $id_sucursal == 214)
            $complemento =' and a.id not in (704) '; 
        $sucursal = $this->db->query("SELECT * FROM Sucursal a where a.id = ".$id_sucursal);
        $alcohol = 0;   
        $result = $this->db->query("select * from (select a.cantidad_x_porcion,a.orden,
            a.id id_articulo,aa.platillo,aa.nombre nombre_articulo,aa.descripcion descripcion_articulo,aa.simbologia,a.logo,a.id_subcategoria,a.id_experiencia,
             a.ubicacion  ubicacion_articulo,IF(a.imagen_min IS NULL OR a.imagen_min = '','',CONCAT('/menudigital/Articulos/'".",a.imagen_min))  ubicacion_articulo_min,(case when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.") in (1)then a.precio_nacional 
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 2 then a.precio_acapulco 
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 3 then a.precio_cc 
            when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 4 then a.precio_tijuana 
            ...
            a.descripcion_imagen,a.detalle_imagen
            from Articulo a
			join Articulo_Idioma aa on aa.id_articulo = a.id
            left join Experiencia e on a.id_experiencia = e.id
            where a.activo = 1 and a.id_categoria = ".$id_categoria. $complemento." )t0 where precio > 0 order by orden");
	...
	public function get_extras($extra,$id_sucursal){
		...
		if($alcohol == 3)
		    $complemento = ' and a.nombre like "%Draft%" ';
		$result = $this->db->query("
			...
			from (
				...
				when (select b.id_precio from Sucursal a join Region b on a.id_region = b.id where a.id = ".$id_sucursal.")= 10 then ROUND(a.precio_colegio_cuota)
				 ...
				 FROM Extra a where a.activo = 1 and FIND_IN_SET(a.id,'".$extra." ".$complemento."')
				)t1 " );
            $response = array();                           
        return $result->result();
    }
